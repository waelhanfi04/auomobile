...
mapData.features.forEach(function(marker) {
  ...
  new mapboxgl.Marker(el)
    ...
    .setHTML('<img src="'+marker.properties.thumbnail+'" alt="'+marker.properties.title+'"/> '+
             '<div class="rim-listing-popup-body"><h5><a href="#" title="'+marker.properties.title+'">' + marker.properties.title + '</a></h5> <span class="listing-price">'+marker.properties.Descr+'</span>'+
              '<p><i class="fas fa-map-signs"></i> ' + marker.properties.description + '</p> <div class="location-popup-meta"> <span><i class="flaticon-dashboard text-orange mr-2"></i>'+marker.properties.Dist+'</span>'+
              '<span>'+marker.properties.model+ '</span><span><i class="fas fa-cog mr-2"></i>'+marker.properties.setting+ '</span> </div></div>'))
    .addTo(map);
});
